...
export class AdminClient {
  ...
  get<T>(endpoint: string, init?: RequestInit): Promise<T> {
    return this.client.request<T>(`/admin/v1/${endpoint.replace(/^\/+/, '')}`, init)
  }

  post<T>(endpoint: string, body: unknown, init?: RequestInit): Promise<T> {
    return this.client.request<T>(`/admin/v1/${endpoint.replace(/^\/+/, '')}`, {
      ...init,
      method: 'POST',
...
export type HealthCheck = { name: string; status: string; message: string | null }
export type Health = { status: string; version: string; checks: HealthCheck[] }
...
export type IssuedCredential = { admin: AdminUser; key: AdminApiKey; token: string }
...
export type EventDetail = { event: EventRecord; deliveries: EventDelivery[] }
export type DatabaseTableRef = { namespace: string; name: string }
export type DatabaseRowCount = { value: number; exact: boolean }
...
export type DatabaseRow = { cells: Record<string, DatabaseCell> }
